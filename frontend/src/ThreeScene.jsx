...
export const ThreeScene = ({ params, data, vizSettings, loading, setLoading, theme }) => {
  ...
  useEffect(() => {
    if (sceneRef.current) {
      sceneRef.current.background = new THREE.Color(theme === 'dark' ? 0x0f0f23 : 0xffffff);
    }
  }, [theme]);
  ...
  useEffect(() => {
    ...
    if (!sceneRef.current) {
      ...
      scene.background = new THREE.Color(theme === 'dark' ? 0x0f0f23 : 0xffffff);
